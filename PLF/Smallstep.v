...
(** **** Exercise: 2 stars, standard (step_deterministic) *)
...
(* FILL IN HERE
   - Does a strong progress theorem hold? Write yes or no and
     ...
     Prove your answer correct in Coq.
*)
...
Proof.
  ...
  - apply multistep_congr_1 with (t2:=t2) in IHeval1.
    apply multistep_congr_2 with (v1:=C v1) in IHeval2.
    +apply multi_trans with (P (C v1) (C v2)).
     *apply multi_trans with (P (C v1) t2). apply IHeval1. apply IHeval2.
...
Proof.
  ...
  induction Hs ; intros; inversion H; subst.
   ...
   -apply E_Plus. apply IHHs in H2. apply H2. apply H4.
   -inversion H0; subst. apply IHHs in H5. apply E_Plus. apply H3. apply H5.
Qed.
(** [] *)
...
Proof.
  ...
  -apply nf_is_value in H0. destruct H0. exists n. split.
    ...
    + apply step__eval with (y).
      *apply H.
      *assert (step_normal_form (C n)).
        { apply value_is_nf. apply v_const. } 
       apply IHmulti in H0. destruct H0. destruct H0. inversion H0. subst. apply H2.
Qed.
...
Proof.
  intros. eapply multi_end_step. apply multi_end_refl. apply H.
Qed.
...
Proof.
  ...
  - apply H.
  - apply IHmulti_end in H1. eapply multi_end_step. apply H1. apply H.
Qed.
(** [] *)
...
Lemma multi_equiv_clos_trans : forall X (R : relation X) x y, 
  multi R x y <-> clos_trans R x y.
Proof.
  intros. split; intros.
  -induction H. 
    + apply t_refl.
    + eapply t_trans. eapply t_step. apply H. apply IHmulti.
  - induction H.
    + apply multi_refl.
    + apply multi_R. apply H.
    + eapply multi_trans. apply IHclos_trans1. apply IHclos_trans2.
Qed. 

Lemma multi_equiv_multi_end : forall X (R : relation X) x y, 
  multi R x y <-> multi_end R x y.
Proof.
  intros. rewrite multi_equiv_clos_trans. split; intros.
  - induction H.
    + apply multi_end_refl.
    + apply multi_end_R. apply H.
    + eapply multi_end_trans. apply IHclos_trans1. apply IHclos_trans2.
  - induction H.
    + apply t_refl.
    + eapply t_trans. apply IHmulti_end. apply t_step. apply H0.
Qed.
(** [] *)
...
(** **** Exercise: 5 stars, standard (determinstic_normalizing_is_strong_normalizing)
    ...
    requires that these aren't any infinite runs.
...
Lemma multi_equiv_multi_n : forall X (R : relation X) x y, 
  multi R x y <-> exists n, multi_n R n x y.
Proof.
  intros. split; intros.
  - induction H.
    + eexists. apply multi_n_refl.
    + destruct IHmulti. exists (S x0). eapply multi_n_step. apply H.  apply H1.
  - destruct H . induction H.
    + apply multi_refl.
    + eapply multi_step. apply H. apply IHmulti_n.
Qed.


Theorem multi_n_trans :
  forall (X : Type) (R : relation X) (n m : nat) (x y z : X),
      multi_n R n x y  ->
      multi_n R m y z ->
      multi_n R (n + m) x z.
Proof.
  intros X R n m x y z G H.
  induction G.
    - (* multi_refl *)simpl. assumption.
    - (* multi_step *)
      apply multi_n_step with y. assumption.
      apply IHG. assumption.
Qed.
 
Theorem multi_n_off_by_1 :
  forall (X : Type) (R : relation X) (n : nat) (x y z : X),
    deterministic R ->
    multi_n R n x y ->
    multi_n R (S n) x z ->
    multi_n R 1 y z.
Proof.
intros X R n. induction n; intros.
  -inversion H0; subst. apply H1.
  - inversion H1; subst; clear H1.
    inversion H0; subst; clear H0.
    apply (H x y0 y1 H3) in H2. subst.
    apply (IHn  y1 y z H H5 H4).
Qed.
...
Proof.
  intros. unfold normalizing in H0. intros t. destruct (H0 t).
  destruct H1. unfold normal_form in H2. apply multi_equiv_multi_n in H1. destruct H1.
  exists (S x0). intros t' contra. apply H2.
  assert (exists n, multi_n R n t t'). 
    { exists (S x0). apply contra. }
  apply multi_equiv_multi_n in H3.
  assert (multi_n R 1 x t'). 
  { eapply multi_n_off_by_1. apply H. apply H1. apply contra. }
  inversion H4; subst; clear H4. exists y. apply H6.
Qed.
...
Inductive bool_ge : bool -> bool -> Prop :=
  | G_eq : bool_ge true true
  | G_le : bool_ge true false.

Theorem exists_normalizing_not_strong_normalizing :
  exists (X : Type) (R : relation X), 
  normalizing R /\ ~ strong_normalizing R.
Proof.
 exists bool, bool_ge. split.
  -intros b. exists false. split.
    + destruct b. apply multi_R. apply G_le. apply multi_refl.
    + intros []. inversion H.
  - intros H. specialize (H true). destruct H. apply (H true). clear H.
    induction x.
    + apply multi_n_refl.
    + eapply multi_n_step. apply G_eq. apply IHx.
Qed.
(** [] *)
...
(** **** Exercise: 5 stars, advanced, optional (strong_normalization)
    ...
    every strongly normalizing relation is normalizing.
