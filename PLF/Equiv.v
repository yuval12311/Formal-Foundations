...
(** **** Exercise: 2 stars, standard (seq_assoc) *)
...
Proof.
  intros.
  split; intros H0; inversion H0; subst; clear H0.
    - assert (Hx : st' =[ X := a ]=> (X !-> st' X ; st')).
      {apply E_Asgn. unfold aequiv in H. 
      simpl in H. symmetry in H. apply H. }
      rewrite t_update_same in Hx.
      apply Hx.
    - unfold aequiv in H. rewrite <- H. simpl. rewrite t_update_same.
      apply E_Skip.
Qed.
...
Proof.
  ...
  apply H1 in H4; apply H2 in H7; apply E_Seq with (st':=st'0);
  try (apply H4); try (apply H7).
Qed.
...
Theorem CIf_congruence : forall b b' c1 c1' c2 c2',
  ...
  cequiv <{ if b then c1 else c2 end }>
         <{ if b' then c1' else c2' end }>.
Proof.
  intros b b1 c1 c1' c2 c2' Hb H1 H2 st st'.
  split; intros H; inversion H; subst; clear H.
  -apply E_IfTrue; [rewrite <- Hb; apply H7| apply H1; apply H8].
  -apply E_IfFalse; [rewrite <- Hb; apply H7| apply H2; apply H8].
  -apply E_IfTrue; [rewrite Hb; apply H7| apply H1; apply H8].
  -apply E_IfFalse; [rewrite Hb; apply H7| apply H2; apply H8].
Qed.
(** [] *)
...
Proof.
intros. apply E_Havoc. Qed.

Example havoc_example2 :
  empty_st =[ skip; havoc Z ]=> (Z !-> 42).
Proof.
intros. apply E_Seq with (st':=empty_st).
    -apply E_Skip.
    -apply E_Havoc.
 Qed.
...
Proof.
  ...
  intros. left. split; intros H.
  - inversion H; subst; clear H. 
    inversion H2; subst; clear H2.
    inversion H5; subst; clear H5.
    apply  E_Seq with (st':=(Y !-> n0; st)).
    +apply E_Havoc.
    +rewrite t_update_permute. apply E_Havoc. discriminate.
  - inversion H; subst; clear H. 
    inversion H2; subst; clear H2.
    inversion H5; subst; clear H5.
...
(** **** Exercise: 4 stars, standard (havoc_copy)
...
intros; right. intros H.
  ...
  inversion H0; subst; clear H0.
   ...
   inversion H6; subst; clear H6.
    simpl. 
    assert ((Y !-> 1; (X !-> 0)) Y = (Y !-> aeval (X !-> n) X; (X !-> n)) Y).
    { rewrite H4. reflexivity. }
 simpl in H0. 
    rewrite t_update_eq in H0.
rewrite t_update_eq in H0.
rewrite t_update_eq in H0.
assert ((Y !-> 1; (X !-> 0)) X = (Y !-> aeval (X !-> n) X; (X !-> n)) X).
    { rewrite H4. reflexivity. }
simpl in H1.
...
Lemma n_leq_Sn : forall n, n <=? n + 1 = true.
Proof.
intros. induction n. 
    - reflexivity.
    -simpl. apply IHn.
...
Lemma p1_may_diverge : forall st st', st X <> 0 ->
  ~ st =[ p1 ]=> st'.
Proof. 
unfold not. intros. remember p1. induction H0; try (intros; inversion Heqc; subst).
  - simpl in H0.  apply f_equal with (f:=negb) in H0. rewrite negb_involutive in H0. simpl in H0.
    apply beq_nat_true in H0. apply H in H0. apply H0.
  -assert (forall n m,  n =? 0 = false -> n <=? m = true -> m =? 0 = false).
   { intros. destruct m.
        -destruct n. apply H1. discriminate.
        -reflexivity. }
   assert (st X <=? st' X = true). 
   {  inversion H0_; subst; clear H0_.
      inversion H4; subst.
      inversion H7; subst; simpl.
      rewrite t_update_eq.
      rewrite t_update_neq.
      rewrite n_leq_Sn; reflexivity. discriminate.
 } 
  apply eqb_neq in H. apply H1 with (m:=st' X) in H.
  apply eqb_neq in H. apply IHceval2. apply H. apply Heqc. apply H2.
Qed.
...
Lemma p2_may_diverge : forall st st', st X <> 0 ->
  ~ st =[ p2 ]=> st'.
Proof.
unfold not. intros. remember p2. induction H0; try (intros; inversion Heqc; subst).
  -simpl in H0. apply eqb_neq in H. rewrite H in H0. discriminate.
  -inversion H0_; subst; simpl in H0_. apply IHceval2. apply H. apply Heqc.
Qed.
(** [] *)
...
Proof. intros st st'. split; intros H; destruct (st X) eqn:E; inversion H; subst;
  try(apply E_WhileFalse; apply H4);
   try(simpl in H2; rewrite E in H2; discriminate).
  -exfalso; apply p1_may_diverge in H. apply H; apply eqb_neq; rewrite E; reflexivity.
    apply eqb_neq. rewrite E. reflexivity.
  -exfalso; apply p2_may_diverge in H. apply H; apply eqb_neq; rewrite E; reflexivity.
    apply eqb_neq. rewrite E. reflexivity.
Qed.
...
Proof. 
 intros. intros H.
  ...
  inversion H6; subst; clear H6.
  inversion H3; subst; clear H3.
  simpl in H5. assert ((Z !-> 1; (X !-> 0; (X !-> 1))) Z = (Z !-> 0; (X !-> 0; (Z !-> 1; (X !-> 1)))) Z).
...
(** **** Exercise: 5 stars, advanced (p5_p6_equiv)
...
Lemma p5_changes_x : forall st st', 
  st =[p5]=> st' -> st' = (X !-> 1 ; st).
Proof.
intros. remember p5. induction H;  try discriminate; inversion Heqc; subst.
  -simpl in H. apply f_equal with (f:=negb) in H. rewrite negb_involutive in H. 
    simpl in H. apply eqb_eq in H. rewrite <- H. rewrite t_update_same. reflexivity.
  -inversion H0; subst. apply IHceval2 in Heqc. rewrite Heqc. rewrite t_update_shadow. reflexivity.
Qed.

Theorem p5_p6_equiv : cequiv p5 p6.
Proof. 
intros. intros st st'. split; intros H.
  -apply p5_changes_x in H. rewrite H. apply E_Asgn. reflexivity.
  -inversion H; subst. simpl.
    destruct ( st X =? 1) eqn:E.
    +apply eqb_eq in E. rewrite <- E. rewrite t_update_same. 
      apply E_WhileFalse. apply eqb_eq in E. simpl. rewrite E; reflexivity. 
