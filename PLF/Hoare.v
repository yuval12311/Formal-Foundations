...
Notation "a - b" := (fun st => mkAexp a st - mkAexp b st) : assertion_scope.
Notation "a * b" := (fun st => mkAexp a st * mkAexp b st) : assertion_scope.
Notation "a ^ b" := (fun st => pow (mkAexp a st) (mkAexp b st)) : assertion_scope.
...
Theorem hoare_asgn_fwd :
  ...
  {{fun st => P (X !-> m ; st)
           /\ st X = aeval (X !-> m ; st) a }}.
Proof.
  intro. unfold hoare_triple. intros.
  inversion H; subst. destruct H0. split.
  -rewrite t_update_shadow. rewrite <- H1. rewrite t_update_same. apply H0.
  -rewrite t_update_eq . rewrite <- H1. rewrite t_update_shadow. rewrite t_update_same. reflexivity.
Qed.
...
Theorem hoare_asgn_fwd_exists :
  ...
  {{fun st => exists m, P (X !-> m ; st) /\
                st X = aeval (X !-> m ; st) a }}.
Proof.
  intro. unfold hoare_triple. intros. inversion H; subst. exists (st X). split.
...
(** **** Exercise: 2 stars, standard, optional (hoare_asgn_examples_2)
...
(** **** Exercise: 2 stars, standard, optional (if_minus_plus)
...
Theorem hoare_if1 : forall P Q (b:bexp) c,
  {{ P /\ b }} c {{Q}} ->
   (fun st => P st /\ (beval st b) = false) ->> Q ->
...
apply hoare_if1.
  ...
  -assn_auto''. destruct H.  apply negb_false_iff in H0.  apply beq_nat_true in H0. lia.
Qed.
