...
(** **** Exercise: 3 stars, standard (parity_formal)
...
Theorem parity_formal : forall m,
...
Example factorial_dec (m:nat) : decorated :=
 <{
    {{ X = m }} 
      Y := 1
    {{ X = m /\ Y = 1 }} ->>
    {{ ap fact m = (ap fact X) * Y }};
      while X <> 0 do
                   {{ ap fact m = (ap fact X) * Y /\ X <> 0 }} ->>
                   {{ ap fact m = (ap fact (X - 1)) * (X * Y) }}
        Y := X * Y
                   {{ ap fact m = (ap fact (X - 1)) * Y  }};
        X := X - 1
                   {{ ap fact m = (ap fact X) * Y  }}
      end
    {{ ap fact m = (ap fact X) * Y /\ ~ (X <> 0) }} ->>
...
Proof. verify.
    -rewrite H. destruct (st X). lia. 
        simpl. replace (fact n + n * fact n) with (S n * fact n).
        replace (st Y + n * st Y) with (S n * st Y).
        replace ((S n) * (fact n)) with ((fact n) * (S n)).
        rewrite mult_assoc. rewrite <- minus_n_O. reflexivity. 
        apply mult_comm. reflexivity. reflexivity. 
    - rewrite  <- eqb_neq in H0. apply not_false_is_true in H0. 
      apply eqb_eq in H0. rewrite H0 in H. simpl in H. 
...
(** **** Exercise: 3 stars, standard, optional (minimum_correct)
...
(** **** Exercise: 3 stars, standard, optional (two_loops)
...
(** **** Exercise: 5 stars, standard (efficient_power) *)

(** Solve the (exam) exercise in the accompanying PDF file.
   Then, formulate your solution in the following.
   Note that is_even(Y) is implemented here using another loop. *)

Definition K : string := "K".

Definition efficient_power (a b : nat) : decorated :=
  <{
  {{ X = a /\ Y = b /\ Z = 1 }}
  ->>
  {{ a ^ b = Z * X ^ Y }}
  while Y <> 0 do
    	{{ a ^ b = Z * X ^ Y  /\ Y <> 0 }}
    	 ->>
      {{ a ^ b = Z * X ^ (Y + 2 * 0)  /\ Y + 2 * 0 <> 0 }}
    K := Y
      {{ a ^ b = Z * X ^ (K + 2 * 0)  /\ K + 2 * 0 <> 0 }};
    W := 0
      {{ a ^ b = Z * X ^ (K + 2 * W)  /\ K + 2 * W <> 0 }};
    while 2 <= K do
      {{ a ^ b = Z * X ^ (K + 2 * W)  /\ K + 2 * W <> 0 /\ 2 <=  K}}
      ->>
      {{ a ^ b = Z * X ^ ((K - 2) + 2 * (W + 1))  /\ (K - 2) + 2 * (W + 1) <> 0 }}
      K := K - 2
      {{ a ^ b = Z * X ^ (K + 2 * (W + 1))  /\ K + 2 * (W + 1) <> 0 }};
      W := W + 1
      {{ a ^ b = Z * X ^ (K + 2 * W)  /\ K + 2 * W <> 0 }}
    end
    {{ a ^ b = Z * X ^ (K + 2 * W)  /\ K + 2 * W <> 0 /\ ~ (2 <=  K)}} ->>
    {{ a ^ b = Z * X ^ K * ((X * X) ^ W)  /\ K + 2 * W <> 0 /\ ~ (2 <=  K)}} ;
    Y := W
    {{ a ^ b = Z * X ^ K * ((X * X) ^ Y)  /\ K + 2 * Y <> 0 /\ ~ (2 <=  K)}};
    if (K = 0) then
	    {{ a ^ b = Z * X ^ K * ((X * X) ^ Y)  /\ K + 2 * Y <> 0 /\ ~ (2 <=  K) /\ K = 0}}
      ->>
      {{ a ^ b = Z * (X * X) ^ Y  /\ K + 2 * Y <> 0 }}
...
Theorem efficient_power_correct : 
 forall a b, outer_triple_valid (efficient_power a b).
Proof. 
  verify.
  - rewrite H. rewrite <- plus_n_O. reflexivity.
  - destruct (st K). discriminate. destruct n. discriminate. lia.
  - destruct (st K). lia. destruct n. lia. discriminate.
  - rewrite H. replace ((st K) + ((st W) + ((st W) + 0))) with (((st K) - 2) + (((st W) + 1) + (((st W) + 1) + 0))) by lia.
    reflexivity.
  - rewrite H. rewrite pow_add_r. rewrite mult_assoc. 
    replace ((st W) + ((st W) + 0)) with (2 * (st W)) by reflexivity.
    rewrite pow_mul_r. rewrite pow_2_r. reflexivity.
  - rewrite H. rewrite pow_0_r. rewrite mul_1_r. reflexivity.
  - rewrite H. assert (st K = 1).
    + destruct (st K). exfalso. apply H2. reflexivity. 
      destruct n. reflexivity. lia.
    +rewrite H3. rewrite pow_1_r. replace ((st Z) * (st X)) with ((st X) * (st Z)) by apply mult_comm.
     reflexivity.
  -rewrite <- eqb_neq in H0. apply not_false_is_true in H0. rewrite eqb_eq in H0.
   rewrite H0 in H. rewrite pow_0_r in H. rewrite mul_1_r in H. rewrite H. reflexivity.
Qed.
(** [] *)
