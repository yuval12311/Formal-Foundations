...
Proof.
  intros. split.
    ...
    - rewrite add_comm in H. destruct m as [|m'].
      +reflexivity.
      +simpl in H. discriminate.
Qed.
(** [] *)
...
Proof.
  intros. unfold not. split.
    -intros. apply or_introl with (B:=Q) in H0 . apply H in H0. apply H0.
    -intros. apply or_intror with (A:=P) in H0 . apply H in H0. apply H0.
Qed.
...
(** **** Exercise: 1 star, standard, optional (dist_not_exists)
...
(** **** Exercise: 3 stars, standard, optional (All)
...
(** **** Exercise: 4 stars, standard (tr_rev_correct)
    ...
    Recall what you saw in the Recitation 02 (also available in Moodle):
...
(** This version of [rev] is said to be _tail-recursive_, because the
    ...
    You can repeat what you did in the recitation, or start from scratch. *)

Theorem rev_append_is_app: forall (X:Type) (l1 l2: list X),
...
Proof.
  ...
  induction l as [|h t IH]. 
    - reflexivity.
    -simpl. assert (tr_rev (h::t) = tr_rev t ++ [h]). {
      unfold tr_rev. simpl. apply rev_append_is_app.
    }
...
Proof.
  ...
  intros. induction n as [|n' IH].
    ...
    -destruct IH as [x H]. destruct (even  (S n')) eqn:E.
       +exists (S x). rewrite even_S in E. apply f_equal with (f:=negb) in E. 
        rewrite negb_negb in E. rewrite E in H. simpl in H. 
        rewrite  double_incr. rewrite H. reflexivity.
       +exists x. rewrite even_S in E. apply f_equal with (f:=negb) in E. 
        rewrite negb_negb in E. rewrite E in H. simpl in H. 
...
Proof.
  intros. split.
    ...
    -unfold not. intros. destruct (x =? y) eqn:E.
      +apply eqb_true in E. apply H in E. exfalso. apply E.
      +reflexivity.
Qed.
(** [] *)
...
Theorem excluded_middle_irrefutable: forall (P : Prop),
  ~ ~ (P \/ ~ P).
Proof.
  unfold not. intros. assert ((P -> False) /\ ((P -> False) -> False)).
    {split.
        +intros HP. apply or_introl with (B:=P->False) in HP. apply H in HP. apply HP.
        +intros HP. apply or_intror with (A:=P) in HP. apply H in HP. apply HP. }
    destruct H0 as [H1 H2]. apply H2 in H1. apply H1.
Qed.
...
(** **** Exercise: 2 stars, standard, especially useful (conjunction_vs_implication) *)
Theorem conjunction_vs_implication : forall (P Q : Prop),
  (P -> ~ Q) <-> ~(P /\ Q).
Proof.
  unfold not. intros. split.
    -intros. destruct H0. apply H in H0. apply H0. apply H1.
    -intros. apply and_intro with (A:=P) in H1. 
        +apply H in H1. apply H1.
        +apply H0.
Qed.
(** [] *)

(** **** Exercise: 3 stars, standard, especially useful (an_equivalence) *)
Theorem an_equivalence : forall (X : Type) (P Q : X -> Prop) (R : Prop),
  (forall x, P x -> Q x -> R) <-> ((exists x, P x /\ Q x) -> R).
Proof.
  split.
    -intros. destruct H0. destruct H0. apply H in H0. apply H0. apply H1.
    -intros. apply H. exists x. split. apply H0. apply H1.
Qed.
(** [] *)
