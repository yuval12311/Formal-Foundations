...
Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof.
  intros. induction l as [|hl tl IH].
    -reflexivity.
    -simpl. rewrite IH. reflexivity.
Qed.
...
Proof.
  intros. induction l1 as [|h1 t1 IH].
...
Fixpoint split {X Y : Type} (l : list (X*Y)) : (list X) * (list Y):=
  match l with
    | nil =>  (nil, nil)
    | (x, y) :: t => let st := split t in (x :: fst st,y :: snd st)
end.
...
(** **** Exercise: 1 star, standard (hd_error_poly)
...
*)
(* FILL IN HERE *)
Fixpoint last_error {X : Type} (l : list X) : option X :=
  match l with 
    | nil => None
    | h :: nil => Some h
    | h :: t => last_error t
...
Example test_last_error1 : last_error [1;2] = Some 2.
 Proof. reflexivity. Qed.
Example test_last_error2 : last_error  [[1];[2]]  = Some [2].
 Proof. reflexivity. Qed.
Example test_last_error3 : @last_error bool nil  = None.
 Proof. reflexivity. Qed.

Definition fold_func {X : Type} (x: X) (ox: option X) :=
  match ox with
    | None => Some x
    | _ => ox
end.

Definition last_error_fold {X : Type} (l : list X) : option X :=
 fold fold_func l None.

Example test_last_error_fold1 : last_error_fold [1;2] = Some 2.
 Proof. reflexivity. Qed.
Exampl                                                                                                                      e test_last_error_fold2 : last_error_fold  [[1];[2]]  = Some [2].
 Proof. reflexivity. Qed.
Example test_last_error_fold3 : @last_error_fold bool nil  = None.
 Proof. reflexivity. Qed.

Lemma last_not_none: forall X (l: list X),
  None = last_error (l) -> l = nil.
Proof.
  intros. induction l as [|h t IH].
    -reflexivity.
    -destruct t as [|h' t'].
      +discriminate.
      +replace (last_error (h :: h' :: t')) with (last_error (h' :: t')) in H by reflexivity.
        apply IH in H. discriminate H.
Qed.
Theorem last_fold: forall X (l: list X),
  last_error_fold l = last_error l.
Proof.
  intros. induction l as [|h t IH].
    -reflexivity.
    -simpl. destruct t as [|h' t'].
      ...
      +rewrite <-IH. 
        assert (
      last_error_fold (h :: h' :: t') 
      = fold_func h (last_error_fold (h' :: t'))) 
      by reflexivity.
      rewrite H. destruct (last_error_fold (h' :: t')).
        *reflexivity.
        *apply last_not_none in IH. discriminate IH.
Qed.  (*This proof is utterly discusting. I'm sure there's a better way to do this.*)
