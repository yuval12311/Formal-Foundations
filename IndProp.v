...
Theorem SSSSev__even : forall n,
  ev (S (S (S (S n)))) -> ev n.
Proof.
  intros. inversion H. apply evSS_ev in H1. apply H1.
Qed.
(** [] *)
...
Theorem ev5_nonsense :
  ev 5 -> 2 + 2 = 9.
Proof.
  intros. apply SSSSev__even in H. inversion H.
Qed.
(** [] *)
...
Proof.
 intros. split.
  -intros. induction H.
    + apply ev_0.
    + apply ev_SS, ev_0.
    +apply ev_sum with (n:=n) (m:=m) in IHev'1. apply IHev'1. apply IHev'2.
  -intros. induction H.
    + apply ev'_0.
    + replace (S (S n)) with (2 + n) by reflexivity . apply ev'_sum. apply ev'_2. apply IHev.
Qed.
(** [] *)
...
Proof.
  intros n m Enm En. induction En.
    ...
    - simpl in Enm. apply evSS_ev in Enm. apply IHEn in Enm. apply Enm.
Qed.
(** [] *)
...
Proof.
  intros n m p  Enm Enp. apply ev_sum with (n:=n+m) (m:=n+p) in Enm.
    -assert (n + m + (n + p) = (n + n) + (m + p)). {
      rewrite add_assoc. 
      replace ((n + m) + n) with  (n + (m + n)).
      replace (m+n) with (n+m).
      replace (n + (n + m)) with  ((n + n) + m).
      rewrite add_assoc. reflexivity.
      rewrite add_assoc. reflexivity.
      rewrite add_comm. reflexivity.
      rewrite add_assoc. reflexivity.
      }
       rewrite H in Enm. apply ev_ev__ev with (n:= n+n) (m:=m+p).
       +apply Enm.
...
Inductive subseq : list nat -> list nat -> Prop :=
  | sub_emp (l : list nat)                                            : subseq [] l
  | sub_cons_right (l1 l2 : list nat) (n: nat) (H : subseq l1 l2)    : subseq l1 (n::l2)
  | sub_cons  (l1 l2 : list nat) (n1 n2: nat) (H : subseq l1 l2)     : subseq (n1::l1) (n2::l2)
.
...
Proof.
  intros. induction H.
    -apply sub_emp.
    -simpl. apply sub_cons_right. apply IHsubseq.
