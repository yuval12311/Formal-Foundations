...
Proof.
  split.
    ...
    - generalize dependent bv. induction b;
        simpl; intros; subst; constructor; repeat(try( apply aeval_iff_aevalR; reflexivity)).
...
Theorem p: forall c1 c2 st st',
     st =[ c1;c2 ]=> st'  ->
     st =[ c1 ]=> st ->
     st =[c2]=> st'.
Proof.
  intros. inversion H. subst. apply ceval_deterministic with (st2:=st'0) in H0. subst. apply H6. apply H3.
Qed.
...
(** **** Exercise: 3 stars, standard, optional (stack_compiler)
...
(** **** Exercise: 3 stars, standard, optional (execute_app) *)
...
(** **** Exercise: 3 stars, standard, optional (stack_compiler_correct) *)
...
Proof.
  intros. inversion H. 
  -inversion H2. subst. inversion H5. reflexivity.
  - inversion H2.     
Qed.
...
Theorem while_stops_on_break : forall b c st st',
  ...
  st =[ while b do c end ]=> st' / SContinue.
Proof.
  intros. apply (E_WhileTrue_Break st st' b c H H0).
Qed.
...
Proof.
  intros. apply (E_Seq c1 c2 st st' st'' SContinue H H0).
Qed.
...
Theorem seq_stops_on_break : forall c1 c2 st st',
  ...
  st =[ c1 ; c2 ]=> st' / SBreak.
Proof.
  intros. apply (E_Seq_Break c1 c2 st st' H).
Qed.
(** [] *)
...
Proof.
 intros. remember (<{while b do c end}>) as whileb.
  induction H; try (intros; inversion Heqwhileb).
