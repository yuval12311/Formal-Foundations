...
Example injection_ex3 : forall (X : Type) (x y z : X) (l j : list X),
  ...
  x = y.
Proof.
  intros. injection H as H1 H2. rewrite <- H2 in  H0. injection H0 as H3.
  transitivity z. 
    -apply H1. 
    -rewrite <- H3. 
  reflexivity.
...
Proof.
  ...
  -intros. destruct m as [|m'].
      ...
      +simpl in H. rewrite <- plus_n_Sm in H. rewrite <- plus_n_Sm in H. 
        injection H as H. apply IH in H. rewrite H. reflexivity.
Qed.
...
Proof.
  ...
  induction l as [|h t IH].
    ...
    -intros. destruct n as [|n'].
      +discriminate H.
      +injection H as H. apply IH in H. simpl. apply H.
Qed. 
(** [] *)
...
(** **** Exercise: 3 stars, advanced, optional (eqb_sym_informal)
   ...
   (* FILL IN HERE
    ...
    [] *)
Search (eqb).
(** **** Exercise: 3 stars, standard (eqb_trans) *)
Theorem eqb_trans : forall n m p,
  n =? m = true ->
  m =? p = true ->
  n =? p = true.
Proof.
  intros. apply eqb_true in H, H0. rewrite H. rewrite <- H0. apply eqb_refl. 
Qed.
(** [] *)
...
Theorem forall_and : forall (X : Type) (test : X -> bool) (h : X) (t : list X),
  forallb test (h :: t) = (test h) && forallb test t.
Proof.
  intros. reflexivity. 
Qed. 
...
Proof. 
  intros. induction l as [|h t IH].
    +reflexivity.
    +simpl. unfold existsb'. rewrite forall_and. rewrite IH. destruct (test h).
      *reflexivity.
      *reflexivity.
Qed.
...
Definition val {X : Type} (xs: option X) (defult: X) : X :=
  match xs with
  | None => defult
  | Some x => x
end.
...
Proof.
  intros.
  assert (x1 = val (Some x1) x1) by reflexivity.
  rewrite H0. rewrite H. reflexivity.
Qed.


Definition head {X : Type} (xs: list X) (defult: X) : X :=
  match xs with
  | [] => defult
  | h :: t => h
end.
...
Proof.
  intros.
  assert (h1 = head (h1::t1) h1) by reflexivity.
  rewrite H0. rewrite H. reflexivity.
Qed.
...
Proof.
  intros.
  assert (t1 = tail (h1::t1)) by reflexivity.
  rewrite H0. rewrite H. reflexivity.
Qed.
(** [] *)

(** **** Exercise: 4 stars, advanced (sum_tail)
...
Fixpoint sum_tail_add (acc : nat) (l : list nat) : nat :=
  match l with
  | [] => acc 
  | h :: t => sum_tail_add (acc+h) t
end.

Definition sum_tail : list nat -> nat :=
  fun l => sum_tail_add 0 l.
...
Proof.
  intros. generalize dependent acc. induction l as [|h t IH].
    -reflexivity.
    -intros. simpl. rewrite  IH. rewrite add_assoc. rewrite add_comm. rewrite add_assoc. reflexivity.
Qed.
...
Proof.
  intros. induction l as [|h t IH].
  -reflexivity.
  -unfold sum_tail. simpl. rewrite add_comm. apply sum_tail_equiv_helper.
Qed. 
